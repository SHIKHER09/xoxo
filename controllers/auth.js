...
export const SignupADM = async (req, res, next) => {
    ...
    try {
        ...
        console.log({...req.body},"2");
        const hash =await bcrypt.hashSync(req.body.password, salt)
        var newUser = await admin.create({ ...req.body, password: hash });
        await newUser.save();
        res.status(200).send(newUser);
    } catch (err) {
        next(err);
...
export const loginADM = async (req, res, next) => {
    try {
        console.log(req.body.email);
...
export const loginwithgoogleADM = async (req, res, next) => {
    try {
        const guser = await admin.findOne({ email: req.body.email })
        if (guser) {
            const token = jwt.sign({ id: guser._id }, process.env.JWT);
